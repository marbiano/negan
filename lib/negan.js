...
Negan.prototype.buildFile = co.wrap(function* (route) {
  ...
  switch (path.extname(route.source)) {
    case '.html':
      file = yield this.readFile(route.source);
      content = yield this.processFile(file, route.options);
      yield this.writeFile(path.join(this.directory, route.destiny, 'index.html'), content);
      break;
    case '.css':
    case '.scss':
      file = yield this.readFile(route.source);
      yield this.writeFile(path.join(this.directory, route.destiny), file);
      break
    case '.js':
    case '.jpg':
    case '.png':
